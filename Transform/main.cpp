...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		/* parameter1：uniform的位置值；
		 * parameter2：告诉OpenGL要发送多少个矩阵
		 * parameter3：询问石佛对矩阵进行置换，即交换行和列，GLM中不需要置换，设置为GL_FALSE
		 * parameter4：矩阵数据。GLM并不是把矩阵存储为OpenGL期望接受的形式，因此要用GLM自带函数 value_ptr 对数据进行变换
		 */
